...
samples <- readxl::read_excel("DNM1_supplemental.xlsx", skip=1) %>% 
...
#GTEx data analysis
#Assign full-length coding transcripts to corresponding exon 10
dnm1b <- c("ENST00000628346","ENST00000341179","ENST00000372923")
dnm1a <- c("ENST00000393594","ENST00000486160")

#GTEx_Analysis_v8_Annotations_SampleAttributesDS.txt downloaded from GTEx portal 
#GTEx_Analysis_v8_Annotations_SampleAttributesDD.xlsx - dictionary for columns
#https://gtexportal.org/home/datasets
gtex_sample_info <- read_tsv("GTEx_Analysis_v8_Annotations_SampleAttributesDS.txt") %>% 
  select(SAMPID, SMTSD)

#GTEx_Analysis_2017-06-05_v8_RSEMv1.3.0_transcript_tpm.gct downloaded from GTEx portal 
#https://gtexportal.org/home/datasets
#Subsetted for gene_id = 'ENSG00000106976.20' i.e. DNM1 
gtex_cortex <- read_tsv("gtex_dnm1.txt") %>% 
  pivot_longer(c(-transcript_id, -gene_id), names_to="SAMPID", values_to="TPM") %>% 
  left_join(gtex_sample_info) %>% 
  filter(SMTSD=="Brain - Cortex") %>% 
  separate(transcript_id, c("transcript"), sep="\\.") %>% 
  mutate(exon = case_when(transcript %in% dnm1a ~ "DNM1a", 
                          transcript %in% dnm1b ~ "DNM1b")) %>% 
  filter(!is.na(exon)) %>% 
  group_by(SAMPID, exon) %>% 
  dplyr::summarize(TPM=sum(TPM)) %>% 
  mutate(TPM_fig = case_when(exon == "DNM1b" ~ -TPM, exon == "DNM1a" ~ TPM)) %>% 
  ungroup() %>% 
  arrange(-TPM) %>% 
  mutate(SAMPID = factor(SAMPID, levels=unique(SAMPID)))

gtex_exon_expand <- gtex_cortex %>% 
  pivot_wider(id_cols=SAMPID, names_from = exon, values_from = TPM) 

#Statistics:t-test of transcript ratio
t.test(gtex_exon_expand$DNM1a/gtex_exon_expand$DNM1b, mu=1)

#Figure S2a
ggplot(gtex_cortex, aes(y=SAMPID, x = TPM_fig, fill = exon)) +
  geom_col() +
  theme_classic() + 
  scale_fill_viridis(option="mako", discrete=T, begin=.25) + 
  geom_vline(xintercept=0, size=.1) + 
...
#Figure S2b
ggplot(gtex_cortex, aes(x=exon, y=TPM, fill=exon)) + 
  geom_violin(aes(fill=exon)) + 
  geom_boxplot(width=.1, aes(color=exon)) + 
  scale_fill_viridis(option="mako", begin=.25, end=1, discrete=T) + 
  scale_color_viridis(option="mako",begin=1,end=.25,discrete=T) + 
